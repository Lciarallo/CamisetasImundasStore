      ...
      try {
        ...
        order = await db.runTransaction(async (tx) => {
          ...
          const patch: Record<string, unknown> = {
            ...
            'payment.qrCodeBase64': charge.qrCodeBase64 ?? null,
            'payment.ticketUrl': charge.ticketUrl ?? null,
...
function orderForClient(order: Record<string, any>): Record<string, any> {
  ...
  return {
    ...
    payment: {
      ...
      ...(payment.qrCodeBase64 ? { qrCodeBase64: payment.qrCodeBase64 } : {}),
      ...(payment.ticketUrl ? { ticketUrl: payment.ticketUrl } : {}),
